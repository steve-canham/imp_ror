...
{
    // Write out summary data for designated version into the designated files
    ...
    export_csv::generate_csv(&params.output_folder, &dv, pool).await?;
...
pub async fn export_all_as_csv(params: &InitParams, pool : &Pool<Postgres>) -> Result<(), AppError>
{
    ...
    let r = export_csv::generate_all_versions_csv(&params.output_folder, pool).await;
    match r {
        Ok(()) => {
            info!("Data summary generated as csv files"); 
            return Ok(())
