...
pub fn edit_config_file(config_file_path: &Path) -> Result<String, AppError>
{
    ...
    **********************************************************************************
    ...
    let curr_quotes = current_config.data_details.double_quotes;
    let p = format!(r#"
    DOUBLE QUOTES
    Please input two characters to represent the left and right double quotes, for use in enclosing
    emphasised titles within names. These must be one of the following four options:
    a) US style 66-99 smart quotes (“”), b) UK style single quotes (‘’), 
    c) French style guillmets («»), or d) German style low / high quotes („“). 
    To accept the current value ('{curr_quotes}') simply press enter, otherwise type the two 
    ...
    while dq == "no_valid_dq".to_string() {
        let users_dq_selection = user_input_or_current(&curr_quotes)?;
