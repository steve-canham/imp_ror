...
N.B. Using cargo to run a program with parameters requires a double hyphen before those parameters (also known as flags, or switches) are listed. This is to clearly differentiate the program's parameters from any that might apply to cargo itself.
...
- importing it into a set of 'src' schema tables, with very little change;</li>
- transforming it, albeit lightly, into a series of 'ppr' schema tables, and </li>
- summarising statistics of the data set and storing those in 'smm' schema tables.</li>
- generating a text file presenting the summary data from the imported version, in a series of tables.</li>

Note that successive use of the -a command will overwrite the data in the src and ppr schema tables, with data from whatever is the most recently imported version, but that the smm schema data for each version is stored permanently.  

#### To extract data as CSV

- cargo run -- -x</i> will generate a set of 7 csv files with the summary data linked to the current (most recently imported) version. Specifying a different version is also possible as long as it has been previously imported and summarised.</li>
- cargo run -- -y</i> will generate a set of 7 csv files with the summary data from all the versions imported to that point.</li>

### Command line arguments

In most cases the commands above are sufficient, but to maximise flexibility a range of parameters are available. The full list is given below. 
...
It is then no longer necessary to provide the data version and date separately. File names such as

- v1.58-2024-12-11-ror-data_schema_v2.json, 
- v1.51-20240821.json, 
- v1.48 20240620.json, and 
- v2.1-2026-01-15-ror-data.json 

all follow the required pattern. The first and last are the forms of the name supplied by ROR, so renaming the file is not necessary.
...
***Note that if any of the four 'set up' flags described below, -i, -c, -k or -m, are used, all other flags and parameters will be ignored. The system will simply rebuild the configuration file and / or lookup and / or summary tables, and then stop.***

***-i***&nbsp;&nbsp;&nbsp;&nbsp;[or --init]. Run all three components of system initialisatuion (equivalent to -c, -k, and -m together). The system will provide prompts to obtain the data for the configuration file, and then create the lookup tables (with data) and summary tables (without data).

***-c***&nbsp;&nbsp;&nbsp;&nbsp;[or --config]. A flag that causes the configuration file to be edited (prompts for each data point are re-presented to the user).

***-k***&nbsp;&nbsp;&nbsp;&nbsp;[or --lookup]. A flag that causes the lookup tables to be regenerated. Generally only used if the code or data for these tables has been revised. 

***-m***&nbsp;&nbsp;&nbsp;&nbsp;[or --summ_setup]. A flag that causes the re-establishment of the summary tables in the smm schema. NOTE - ANY EXISTING DATA IN THOSE TABLES WILL BE DESTROYED. 
